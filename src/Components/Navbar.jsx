            ...
            <div className='flex justify-between items-center w-full h-[100px] md:h-[120px]'>
                ...
                <div className='flex items-center float-to-right'>
                    <ul className='hidden md:flex gap-2 md:gap-4 lg:gap-8 font-bold text-[10px] sm:text-xs md:text-sm lg:text-lg mr-3 sm:mr-6 md:mr-10 lg:mr-14'>
                        <Link to='/' exact className={`cursor-pointer hover:text-red-600 pl-1 md:pl-2 font-poppins ${isActive('/') ? "text-red-500 border-l-2 border-red-500" : "text-gray-800"}`}>HOME</Link>
                        <Link to='/product' exact className={`cursor-pointer hover:text-red-600 pl-1 md:pl-2 font-poppins ${isActive('/product') ? "text-red-500 border-l-2 border-red-500" : "text-gray-800"}`}>PRODUCTS</Link>
                        <Link to='/about-us' className={`cursor-pointer hover:text-red-600 pl-1 md:pl-2 font-poppins ${isActive('/about-us') ? "text-red-500 border-l-2 border-red-500" : "text-gray-800"}`}>ABOUT US</Link>
                        <Link to='/contact-us' className={`cursor-pointer hover:text-red-600 pl-1 md:pl-2 font-poppins ${isActive('/contact-us') ? "text-red-500 border-l-2 border-red-500" : "text-gray-800"}`}>CONTACT US</Link>
                    </ul>
                    <div className='md:flex items-center hidden gap-2 lg:gap-5'>
                        <Link to='/cart'>
                            <div className='relative'>
                                <TiShoppingCart className={`hover:text-red-600 cursor-pointer ${isActive('/cart') || isActive('/checkout') ? 'text-red-500' : 'text-gray-700'}`} size={35}/>
                                {getTotalItems() > 0 && (
                        ...
                        <Link to='/profile'>
                            <FaUserLarge className={`hover:text-red-600 cursor-pointer ${isActive('/profile') ? 'text-red-500' : 'text-gray-700'}`} size={25}/>
                        </Link>
                    </div>
